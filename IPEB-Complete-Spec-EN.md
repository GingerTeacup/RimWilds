...
## Complete Mod Specification - v2.2 Final
...
8. [Mod Compatibility](#mod-compatibility)
...
Create a realistic and intelligent ecological system that:

1. **Makes predation intelligent** - Predators evaluate benefit/risk before hunting
2. **Avoids mutual death** - No more "suicide" combats where both die
3. **Creates realistic omnivory** - Herbivores occasionally eat meat, carnivores eat plants
4. **Manages scientific scavenging** - Carcasses at different stages, toxins, bone digestion
5. **Spawn ecologically coherent** - Animals spawn in correct biomes/altitudes
6. **Supports entire modded ecosystem** - Compatible with 250+ species and modded biomes
7. **Extensible & collaborative** - Easy for community to add/modify species

## Problems Solved

### ❌ Before (Vanilla RimWorld + Mods)
- Ornithosuchus hunts Tanystropheus → both die (stupid!)
- Cat hunts Thrumbo → cat suicide
- Hippo kills all VAE carnivores (overpowered)
- Small dinosaurs hunt herbivores 6× larger
- No realistic omnivory
- Vanilla scavenging ignored (no carcass management)
- Animals spawn everywhere (Lion in tundra, Penguin in desert)

### ✅ After (With IPEB)
- Predators assess before hunting (intelligence + hunger)
- No suicidal combats (except if very hungry/malnourished)
- Hippo has correct stats, other predators cautious
- Small dinosaurs avoid herbivores too large
- Realistic omnivory (herbivore 5-15% meat, carnivore 2-8% plants)
- Complete scavenging (carcass 0-365 days, toxins, nutritious bones)
- Ecological spawn (Lion in savanna 90%, tundra 0%)
...
## Phase 1: Fast Triage (Lightweight Filter)

**Frequency:** Every 360 ticks (6 seconds), 1 random predator

**Objective:** Create fast list of "interesting" prey without complex calculations

**Parameters Evaluated:**
1. **Body Size Range** - Prey not too small (little food) nor too large (uncontrollable)
2. **Speed Range** - Predator can catch prey
3. **AI Threat Level** (approximate) - Prey not too dangerous
4. **Geographic Proximity** - Prey accessible

**Output:** "Core Prey" list (typically 10-50 candidates)

**Caching:**
- Key: `predator_id`
- Value: `List<prey_id>`
- Update: Every 360 ticks (1 predator/time)
- Invalidation: When animal spawns/dies/moves

---

## Phase 2: Detailed Evaluation (Complex Simulation)

**Frequency:** Every 500 ticks (after Phase 1 sufficiently filled)

**Objective:** Calculate detailed Hunting Worthiness for Top 5 prey
...
Example:
  ...
  - Injured Rat (Simple, 80%): 0.44 accuracy, ±28% error
  - Thrumbo (Advanced, 100%): 0.95 accuracy (near-perfect)
```
...
- Update: Every 500 ticks (top 5 only)
- Invalidation: Prey takes >20% damage, predator feeds

---

## Phase 3: Real Decision (Immediate When Hungry)
...
Hunger Level:
  70%+: ×1.0 (satiated)
  50-70%: ×1.2 (starting hunger)
  30-50%: ×1.5 (significant hunger)
  15-30%: ×2.0 (very hungry)
  <15%: ×3.0 (critical)
...
Nutrition Efficiency: 40-70% of normal (meat harder to digest for herbivores)
```
...
Occasional Plant Eating (2-8% chance):
  ...
  - Cat: 2%, prefer grass
  - Bear: 60%, prefer fruits (TRUE OMNIVORE)
...
### Toxin Tolerance per Species (Scientific)

| Species | Gastric pH | Tolerance | Preferred (days) | Food Poison % | Bones % |
|---------|-----------|-----------|-----------------|--------------|---------|
| **Hyena** | 1.5 | Exceptional | 1-10 | 2% | 120% |
| **Bear** | 1.5 | Exceptional | Any | 1% | 130% |
| **Lion** | 2.0 | High | 0-2 | 8% | 70% |
| **Wolf** | 2.0 | High | 0-1 | 12% | 50% |
| **Rat** | 2.0 | Immune | Any | 0% | 20% (gnaw) |
| **Cat** | 2.5 | Low | 0-0.25 | 25% | 0% |
| **Jackal** | 2.5 | Moderate | 0-0.5 | 20% | 20% |
...
## RDS Implementation per Species
...
**Continuous Format (Not Discrete):**

| Variable | Math Function | Min | Optimal Min | Optimal Max | Max | Notes |
|----------|---------------|----|------------|------------|-----|-------|\n| Temperature | GAUSSIAN | -30 | 0 | 15 | 35 | Center=7.5, Spread=10 |
| Vegetation | BETA | 0.30 | 0.40 | 0.70 | 0.85 | Optimal plateau |
| Open Terrain | BETA | 0.05 | 0.20 | 0.40 | 0.70 | Needs openness |
...
20°N and 20°S → IDENTICAL environment
  ...
  - Same day length
  - Same sun angle
  - Different seasons (inverted) but equivalent conditions
...
### Civilization Expansion (Every 90 days)
```
...
## Part 5: Multi-Layer Altitude (3D)

**Odyssey DLC + Layered Atmosphere Support:**
```
...
## Avatar Pandora - Advance Preparation

**Status:** Avatar mod doesn't exist yet, designing now
...
When Avatar mod releases:
1. Mod creator adds species to XML
2. Community fills habitat data
3. Google Sheets updated
4. Git pull request merged
5. IPEB auto-supports all Pandora species

---

# 8. MOD COMPATIBILITY

## Supported Animal Mods (18+)

```
VANILLA ANIMALS EXPANDED (60+ species)
  ├─ VAE Reptiles
  ├─ VAE Insects
  ├─ VAE Arachnids
  └─ VAE Aquatic

JURASSIC RIMWORLD (50+ dinosaurs)
  ├─ Theropods (T-Rex, Velociraptor, etc.)
  ├─ Sauropods (Brachiosaurus, etc.)
  ├─ Ornithopodes (Triceratops, etc.)
  └─ Special (Pterosaurs)

MEGAFAUNA (39 prehistoric)
...
+ 13 OTHER MODS (Little Critters, HC Animal, Dog breeds, etc.)

TOTAL: 250-300+ species
```

---

## Supported Biome/Terrain Mods

```
REGROWTH (Biome progression)
  └─ Old Growth Forest, Secondary Forest, Depleted Soil
  
REALISTIC PLANETS (Variable size)
  └─ Dynamic map size detection
...
✅ **Gameplay:**
- No more "suicide hunt" combats (mutual death)
